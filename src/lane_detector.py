...
class LaneDetector(object):
    ...
    def image_cb(self, msg):
        ...
        captured_frame_g = cv2.inRange(
            captured_frame_hsv, np.array([0, 0, 120]), np.array([40, 10, 128]))
        captured_frame_w = cv2.inRange(
            captured_frame_hsv, np.array([0, 0, 245]), np.array([179, 255, 255]))

        captured_frame_grey = cv2.bitwise_or(
            captured_frame_g, captured_frame_w)

        # Second blur to reduce more noise, easier circle detection
        captured_frame_grey = cv2.GaussianBlur(
            captured_frame_grey, (5, 5), 2, 2)
        _, lanes = cv2.threshold(
            captured_frame_grey, 128, 255, cv2.THRESH_BINARY)
